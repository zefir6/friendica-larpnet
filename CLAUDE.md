...
| `addon/larpnet_matrix/` | Login bridge to the self-hosted Synapse/Element Matrix chat (see `larpnet-config`'s test/staging stack for the actual homeserver deployment). Mints a 60s HS256 JWT per larpnet user that clients trade at Synapse's `/login` (`org.matrix.login.jwt`), so no second chat password exists. `GET /larpnet_matrix` embeds the chat widget (Element in an iframe) for web users, optionally with `?dm=<nickname>` (see `src/Model/Profile.php` below) to land straight on a DM instead of Element's default view; `POST /larpnet_matrix` returns identity + JWT as JSON for OAuth2-authenticated native apps. Inert unless the `LARPNET_MATRIX_*` env vars are set. `larpnet_matrix_localpart()` is the single source of truth for "nickname → Matrix localpart", shared with `src/Model/Profile.php` via `require_once` so the two can't drift. `larpnet_matrix_sync_profile()` also pushes displayname + avatar to the Matrix profile (throttled hourly, pconfig-tracked) via a server-side login against `LARPNET_MATRIX_INTERNAL_URL` — a separate, internal-network-only URL from `LARPNET_MATRIX_HOMESERVER_URL`, since the container has no general internet egress and can't reach the latter (a public/tunnel address) itself. |
