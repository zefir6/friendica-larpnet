...
| `src/Model/Profile.php` | Patched `getVCardHtml()` to add a "Chat" deep link (`larpnet_matrix?dm=<nickname>`) to another local user's `/profile/<nickname>` page — this route only ever exists for genuinely local accounts, so there's no risk of linking to a Matrix identity the JWT bridge never provisioned. `getMatrixChatLink()` reaches into `addon/larpnet_matrix/` via `require_once` to reuse its settings/localpart checks, same pattern as `src/Worker/FcmPush.php` + `addon/larpnet_fcm/` below. |
