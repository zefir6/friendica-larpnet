...
| `src/Model/Post/Question.php` | Adds `getByURIId()`, `validatePoll()`, `createFromOptions()`, `vote()` and the `VOTE_*`/`MAX_OPTIONS`/`MAX_CHARACTERS_PER_OPTION`/`MIN_EXPIRATION`/`MAX_EXPIRATION` constants -- the single source of truth for poll-creation limits and voting rules |
...
| `view/templates/item/compose.tpl` | Adds the poll-creation fields (up to `Question::MAX_OPTIONS` option inputs, multiple-choice checkbox, expiry select) to the "post" compose form's permissions panel |
