...
<script type="text/javascript">
	$(function() {
		...
		let $visibility_local_panel  = $('#visibility-local-panel-{{$input_group_id}}');
		...
		let $visibility_local_radio  = $('#visibility-local-{{$input_group_id}}');
		let $visibility_custom_radio = $('#visibility-custom-{{$input_group_id}}');

		// Frio specific — register ALL three panels with the same parent so Bootstrap's
		// accordion manages hide/show atomically; explicit collapse('hide') calls before
		// collapse('show') set transitioning=1 and cause Bootstrap to bail out silently.
		if ($.fn.collapse) {
			$visibility_public_panel.collapse({parent: '#visibility-accordion-{{$input_group_id}}', toggle: false});
			$visibility_local_panel.collapse({parent: '#visibility-accordion-{{$input_group_id}}', toggle: false});
		...
		$visibility_public_radio.on('change', function (e) {
			...
			$visibility_local_panel.prop('disabled', true);
		...
		$visibility_local_radio.on('change', function(e) {
			if ($.fn.collapse) {
				$visibility_local_panel.collapse('show');
			}

			$visibility_public_panel.prop('disabled', true);
			$visibility_local_panel.prop('disabled', false);
			$visibility_custom_panel.prop('disabled', true);
		...
		$visibility_custom_radio.on('change', function(e) {
			...
			// Lazily initialize tagsinput the first time the custom panel becomes
			// visible — initializing inside a hidden container causes zero dimensions.
			if (!tagsInputReady) {
				if ($.fn.collapse) {
					$visibility_custom_panel.one('shown.bs.collapse', initTagsInput);
				} else {
					initTagsInput();
				}
			}
		...
		acl.initialize();
		// Initialize before the pre-population loop: without this, .get() returns [] silently
		// and pre-selected tags (e.g. Followers when switching Local → Private) never appear.
		acl_circles.initialize();
		acl_contacts.initialize();
		...
		// Shared tagsinput options for both allow and deny inputs.
		function makeTagsInputOpts() {
			return {
				confirmKeys: [13, 44],
				freeInput: false,
		...
		// Guard so initTagsInput() is idempotent.
		var tagsInputReady = false;

		function initTagsInput() {
			if (tagsInputReady) return;
			tagsInputReady = true;

			$acl_allow_input.tagsinput(makeTagsInputOpts());
			$acl_deny_input.tagsinput(makeTagsInputOpts());
		...
		// If the custom panel is already the active visibility, it's visible on
		// page load — initialize immediately rather than waiting for a panel open.
		{{if $visibility == 'custom'}}initTagsInput();{{/if}}
	});
</script>
