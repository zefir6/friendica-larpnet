...
function larpnet_scheme_get_list(): array
{
	$schemes = [
	...
	$schemes[LARPNET_CUSTOM_SCHEME] = DI::l10n()->t('Custom');

	return $schemes;
}

function larpnet_scheme_get_current()
{
	$available = array_keys(larpnet_scheme_get_list());

	$scheme = DI::config()->get('larpnet', 'scheme') ?: DI::config()->get('larpnet', 'schema');
...
function larpnet_scheme_get_current_for_user(int $uid)
{
	$available = array_keys(larpnet_scheme_get_list());
